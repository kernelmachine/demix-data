...
def extract_features_( model, batch_size, fname, domains=None, path_to_file=None, files=None, sample=None, path_to_data_bins=[], path_to_clusterer=None):
    # kmeans = MiniBatchKMeans(n_clusters=8)
    
    
    accelerator = Accelerator()
    kmeans = KMeans(num_clusters=8, device=accelerator.device)
    kmeans = kmeans.load(Path(path_to_clusterer))
    ...
    if path_to_data_bins:
        ...
        texts = pd.read_csv(path_to_file)
        ...
        dataset = ConcatDataset(datasets)
        accelerator.print(f"Size of dataset: {len(dataset)}")
    else:
    ...
    if path_to_data_bins:
        ...
        data = torch.utils.data.DataLoader(dataset, num_workers=16, batch_size=batch_size)
    ...
    if path_to_data_bins:
        for batch in tqdm(data, disable=not accelerator.is_local_main_process):
            ...
            with torch.no_grad():
                ...
                all_feats = accelerator.gather(feats).cpu().numpy()
                # kmeans.partial_fit(all_feats.numpy())
                all_ids = accelerator.gather(ix).cpu().numpy()
                if len(features) % 100 == 0 and len(features) > 0:
                    features = np.concatenate(features, 0)
                    ids = np.concatenate(ids, 0)
                    np.save(fname +  f"/features_{shard_idx}.pt", features)
                    np.save(fname + f"/ids_{shard_idx}.pt", ids)
                    shard_idx += 1
                    features = []
                    ids = []
                else:
        ...
        # kmeans = MiniBatchKMeans(n_clusters=8)
        counter = 0
        for filenames, text, _, _ in tqdm(data, disable=not accelerator.is_local_main_process):
            input_ids = tokenizer.batch_encode_plus(list(text),
                            add_special_tokens=True,
                        truncation=True,
                        max_length=512,
            ...
            with torch.no_grad():
                ...
                #     features.append(all_feats)
                #     ids.append(all_ids)
                # kmeans = kmeans.partial_fit(all_feats.cpu().numpy())
                # kmeans.fit(all_feats.to(kmeans.device), online=True, iter_limit=1, iter_k=counter)
                # counter += 1
                clusters = kmeans.predict(feats.to(accelerator.device), distance='euclidean')
                for cluster, fname in zip(clusters, filenames):
                    Path(Path("reclustered") / f"{cluster}").mkdir(parents=True, exist_ok=True)
                    with open(Path("reclustered") / f"{cluster}" / f"{str(torch.distributed.get_rank())}.txt", 'a+') as f:
                        f.write(fname + "\n")
                # all_feats = accelerator.gather(feats).cpu()
                # all_ids = accelerator.gather(ix).cpu()
                # features.append(all_feats)
                # ids.append(all_ids)
            # accelerator.print(f"shard {ix} complete.")
        # kmeans.save("model.pkl")
        # save_model(kmeans, "model")
    # features = torch.cat(features, 0)
    # ids = torch.cat(ids, 0)
    # np.save(fname +  "/features.pt", features)
    # np.save(fname + "/ids.pt", ids)

def extract_features(model, batch_size, num_gpus, fname, domains=None, path_to_file=None, path_to_data_bins=[], master_addr="127.0.0.1", master_port=29500, notebook=False, sample=None, path_to_clusterer=None):
    if notebook:
        ...
        extract_features_(model, batch_size, fname, domains=domains, path_to_file=path_to_file, path_to_data_bins=path_to_data_bins, sample=sample, path_to_clusterer=path_to_clusterer)
