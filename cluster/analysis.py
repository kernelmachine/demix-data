...
class ClusterAnalysis:
    ...
    def plot_tsne(self,
        ...
        if ground_truth is not None:
            ground_truth_color_palette = sns.color_palette('Paired', max(np.unique(ground_truth)+1))
        ...
        cluster_colors = [color_palette[x] if x >= 0
                          ...
                          for x in model_labels_]
        
        if ground_truth is not None:
            cluster_colors_ground_truth = [ground_truth_color_palette[x] if x >= 0
                                          else (0.5, 0.5, 0.5)
