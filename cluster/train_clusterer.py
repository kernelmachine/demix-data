...
def extract_features_( model, batch_size, fname, domains=None, path_to_file=None, files=None, sample=None, path_to_data_bins=[], num_clusters=None):
    ...
    if path_to_data_bins:
        ...
        data = torch.utils.data.DataLoader(dataset, num_workers=10, batch_size=batch_size)
    ...
    if path_to_data_bins:
        ...
        for ix, text in tqdm(data, disable=not accelerator.is_local_main_process):
            ...
            with torch.no_grad():
                ...
                kmeans.fit(all_feats.to(kmeans.device), online=True, iter_limit=10, iter_k=counter, balanced=True)
                counter += 1
        ...
        for filenames, text, _, _ in tqdm(data, disable=not accelerator.is_local_main_process):
            ...
            with torch.no_grad():
                out = model(**input_ids)
                feats = torch.mean(out[0], 1)
                all_feats = accelerator.gather(feats)
                if torch.distributed.get_rank() == 0:
                    kmeans.fit(all_feats, online=True, iter_limit=10, iter_k=counter, tqdm_flag=False)
                accelerator.wait_for_everyone()
                counter += 1
    if torch.distributed.get_rank() == 0:
...
def extract_features(model, batch_size, num_gpus, fname, domains=None, path_to_file=None, path_to_data_bins=[], master_addr="127.0.0.1", master_port=29500, notebook=False, sample=None, num_clusters=None):
    if notebook:
        ...
        extract_features_(model, batch_size, fname, domains=domains, path_to_file=path_to_file, path_to_data_bins=path_to_data_bins, sample=sample, num_clusters=num_clusters)
