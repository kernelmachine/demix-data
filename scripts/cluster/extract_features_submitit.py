...
def extract(input_df, output_file, model_path, batch_size, submitit=False):
    ...
    tqdm.pandas()
    input_df['text'] = input_df.text.progress_apply(bpe.decode)
    ...
    for batch_json in tqdm(df_iterator,
        ...
        if  model_path == 'tfidf':
            ...
            vectors.append(hf_extract_features(lines, model, tokenizer))
    ...
    if args.model == 'tfidf':
        with open(str(output_file) + ".tfidf.pkl",  'wb+') as f:
...
if __name__ == '__main__':
    ...
    text = [int(x.strip()) for x in tqdm(z.split())]
    text = list(tqdm(chunks(text, args.seq_len)))
