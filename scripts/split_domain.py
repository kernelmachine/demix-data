...
def write_split(domain: str,
    ...
    if clusterer:
        ...
        filehandle = [open(output_dir / f"{split}.{domain}.jsonl", "w+")]
        curr_tokens = {0: 0}
    ...
    for id, fname, text,_, _ in pbar:
        ...
        for item in iter_:
            ...
            token_count = TOKEN_COUNTS_DEMIX[domain][f'num_{split}_tokens'] // num_random_splits
            if sum(curr_tokens.values()) > token_count:
                if not written:
                    count_ = 0
                    item = " ".join(doc.split()[:token_count])
                    if clusterer:
            ...
            if clusterer:
                ...
                filehandle[0].write(json.dumps({"text": doc.strip()}) + "\n")
                curr_tokens[0] += len(doc.split())
            written = True
        if done:
            break
...
if __name__ == '__main__':
    ...
    print(TOKEN_COUNTS_DEMIX[domain])
    ...
    for i in range(8):
        output_dir = args.output_dir / f"_split_{i}"
        ...
        with open(output_dir / f"train_ids_{domain}.txt", "w+") as f:
            for file in train_ids:
                f.write(str(file) + "\n")
        with open(output_dir / f"train_token_counts_{domain}.txt", 'w+') as f:
            json.dump(num_train_tokens, f)
        ignore_ids = {**ignore_ids, **train_ids}
